...
void print_rq () {
	...
	printf("\033[0;32mCurrent State of Run Queue: \n\033[0m");
	curr = rq->head;
	if (curr){
			// printf("%s", curr->threadinfo->processName);

			printf("%s, (goodness: %f, exp: %.2lf, burst: %.2lf got_cpu: %lld, lost_cpu: %lld)\n", curr->thread_info->processName, curr->goodness, curr->expected_burst, curr->burst, curr->got_cpu, curr->lost_cpu);
			// printf("%p", curr);
		}
